...
enum drive_status drive_to_target(struct rover_state *rover,
  ...
  for (int step = 0; step < MAX_DRIVE_STEPS; step++) {
    ...
    float linear_velocity = MAX_LINEAR_VELOCITY;
    ...
    struct wheel_velocity velocity;
    velocity.left = (linear_velocity - (angular_velocity * WHEEL_SEPARATION / 2.0f)) / WHEEL_RADIUS;
    velocity.right = (linear_velocity + (angular_velocity * WHEEL_SEPARATION / 2.0f)) / WHEEL_RADIUS;
